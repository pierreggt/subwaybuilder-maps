@echo off
set SCRIPT_DIR=%~dp0
set TARGET=%SCRIPT_DIR%..\..\cities\data\AUH
set VERSION=1.30.0
echo [Abu Dhabi Mod] Copying data files to cities\data\AUH...
if not exist "%TARGET%" mkdir "%TARGET%"
copy /Y "%SCRIPT_DIR%data\AUH\*" "%TARGET%\" >nul
echo [Abu Dhabi Mod] Data files copied successfully.
if not exist "%SCRIPT_DIR%pmtiles.exe" (
    echo [Abu Dhabi Mod] Downloading pmtiles.exe...
...
echo [Abu Dhabi Mod] Starting tile server on port 8080...
