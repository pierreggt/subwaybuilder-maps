@echo off
set SCRIPT_DIR=%~dp0
set TARGET=%SCRIPT_DIR%..\..\cities\data\DOH
set VERSION=1.30.0
echo [Doha Mod] Copying data files to cities\data\DOH...
if not exist "%TARGET%" mkdir "%TARGET%"
copy /Y "%SCRIPT_DIR%data\DOH\*" "%TARGET%\" >nul
echo [Doha Mod] Data files copied successfully.
if not exist "%SCRIPT_DIR%pmtiles.exe" (
    echo [Doha Mod] Downloading pmtiles.exe...
...
echo [Doha Mod] Starting tile server on port 8080...
