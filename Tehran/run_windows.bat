@echo off
set SCRIPT_DIR=%~dp0
set TARGET=%SCRIPT_DIR%..\..\cities\data\THR
set VERSION=1.30.0
echo [Tehran Mod] Copying data files to cities\data\THR...
if not exist "%TARGET%" mkdir "%TARGET%"
copy /Y "%SCRIPT_DIR%data\THR\*" "%TARGET%\" >nul
echo [Tehran Mod] Data files copied successfully.
if not exist "%SCRIPT_DIR%pmtiles.exe" (
    echo [Tehran Mod] Downloading pmtiles.exe...
...
echo [Tehran Mod] Starting tile server on port 8080...
