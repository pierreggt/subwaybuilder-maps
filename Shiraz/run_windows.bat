@echo off
set SCRIPT_DIR=%~dp0
set TARGET=%SCRIPT_DIR%..\..\cities\data\SYZ
set VERSION=1.30.0
echo [Shiraz Mod] Copying data files to cities\data\SYZ...
if not exist "%TARGET%" mkdir "%TARGET%"
copy /Y "%SCRIPT_DIR%data\SYZ\*" "%TARGET%\" >nul
echo [Shiraz Mod] Data files copied successfully.
if not exist "%SCRIPT_DIR%pmtiles.exe" (
    echo [Shiraz Mod] Downloading pmtiles.exe...
...
echo [Shiraz Mod] Starting tile server on port 8080...
