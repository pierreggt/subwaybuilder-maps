@echo off
set SCRIPT_DIR=%~dp0
set TARGET=%SCRIPT_DIR%..\..\cities\data\MKJ
set VERSION=1.30.0
echo [Mecca & Jeddah Mod] Copying data files to cities\data\MKJ...
if not exist "%TARGET%" mkdir "%TARGET%"
copy /Y "%SCRIPT_DIR%data\MKJ\*" "%TARGET%\" >nul
echo [Mecca & Jeddah Mod] Data files copied successfully.
if not exist "%SCRIPT_DIR%pmtiles.exe" (
    echo [Mecca & Jeddah Mod] Downloading pmtiles.exe...
...
echo [Mecca & Jeddah Mod] Starting tile server on port 8080...
