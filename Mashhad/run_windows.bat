@echo off
set SCRIPT_DIR=%~dp0
set TARGET=%SCRIPT_DIR%..\..\cities\data\MHD
set VERSION=1.30.0
echo [Mashhad Mod] Copying data files to cities\data\MHD...
if not exist "%TARGET%" mkdir "%TARGET%"
copy /Y "%SCRIPT_DIR%data\MHD\*" "%TARGET%\" >nul
echo [Mashhad Mod] Data files copied successfully.
if not exist "%SCRIPT_DIR%pmtiles.exe" (
    echo [Mashhad Mod] Downloading pmtiles.exe...
...
echo [Mashhad Mod] Starting tile server on port 8080...
