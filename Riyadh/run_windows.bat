@echo off
set SCRIPT_DIR=%~dp0
set TARGET=%SCRIPT_DIR%..\..\cities\data\RUH
set VERSION=1.30.0
echo [Riyadh Mod] Copying data files to cities\data\RUH...
if not exist "%TARGET%" mkdir "%TARGET%"
copy /Y "%SCRIPT_DIR%data\RUH\*" "%TARGET%\" >nul
echo [Riyadh Mod] Data files copied successfully.
if not exist "%SCRIPT_DIR%pmtiles.exe" (
    echo [Riyadh Mod] Downloading pmtiles.exe...
...
echo [Riyadh Mod] Starting tile server on port 8080...
