@echo off
set SCRIPT_DIR=%~dp0
set TARGET=%SCRIPT_DIR%..\..\cities\data\BEY
set VERSION=1.30.0
echo [Beirut Mod] Copying data files to cities\data\BEY...
if not exist "%TARGET%" mkdir "%TARGET%"
copy /Y "%SCRIPT_DIR%data\BEY\*" "%TARGET%\" >nul
echo [Beirut Mod] Data files copied successfully.
if not exist "%SCRIPT_DIR%pmtiles.exe" (
    echo [Beirut Mod] Downloading pmtiles.exe...
...
echo [Beirut Mod] Starting tile server on port 8080...
